...
/* Sends an ofp_multipart_request message  
 *
 * Does not block: returns EAGAIN if the messages cannot be immediately
 * accepted for transmission. */
int Openflow_connection::send_stats_request(ofp_multipart_types type)
{
    struct ofp_multipart_request osr;

    /* Send OFPT_MULTIPART_REQUEST. */
    osr.header.type    = OFPT_MULTIPART_REQUEST;
